...
What makes this temporal is Zep's [bi-temporal model](https://arxiv.org/abs/2501.13956), which tracks two separate timelines: when events actually happened in the real world (event time) and when information was recorded in the system (transaction time). Each edge in the graph includes validity metadata. When a fact changes, Zep doesn't overwrite the old information. Instead, it marks the old fact's validity period as having ended and creates a new fact with its own validity period.
...
| Reduces tokens by 90% <br> Production-ready with a hybrid architecture <br> (vector + key-value + graph) | More sophisticated than simple extraction | Memory lives in context window <br> Shareable across agents |

**Mem0:** Drop-in memory layer for stateless agents. [Reduces tokens by 90%](https://arxiv.org/abs/2504.19413), production-ready with a hybrid architecture (vector + key-value + graph).

**The AI Memory SDK:** Pluggable memory using background agents for intelligent processing. More sophisticated than simple extraction.
...
**The AI Memory SDK:** Simple drop-in memory without temporal tracking. Agent-based processing.
...
**Letta AI Memory SDK:** Drop-in memory with agent-based processing. No knowledge graph construction.
