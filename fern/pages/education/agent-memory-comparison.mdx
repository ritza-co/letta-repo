...
<img src="https://raw.githubusercontent.com/letta-ai/letta/refs/heads/main/fern/images/agent-memory-comparison/stateful_agents_dark.webp" />
...
<img src="https://raw.githubusercontent.com/letta-ai/letta/refs/heads/main/fern/images/agent-memory-comparison/02-mem0-architecture.svg" />
...
At the core of Zep is Graphiti, an open-source library for building temporal knowledge graphs. When you add conversation data to Zep, Graphiti extracts entities (people, places, concepts) and relationships (works at, lives in, prefers) and stores them as a graph.
...
<img src="https://raw.githubusercontent.com/letta-ai/letta/refs/heads/main/fern/images/agent-memory-comparison/03-zep-temporal-graph.svg" />
...
<img src="https://raw.githubusercontent.com/letta-ai/letta/refs/heads/main/fern/images/agent-memory-comparison/04-cognee-semantic-graph.svg" />

### Building Knowledge Graphs

Cognee combines two storage layers:

**Vector Database**: For semantic similarity search. Supports Weaviate, Qdrant, LanceDB, and others.

**Graph Database**: For relationship modeling. Supports Neo4j and Kuzu.
...
Letta takes a fundamentally different approach from the other solutions in this article. While Mem0, Zep, and Cognee add memory capabilities to stateless agents, Letta is a general-purpose memory programming system for building stateful agents where memory is embedded into the agent's core architecture.
...
<img src="https://raw.githubusercontent.com/letta-ai/letta/refs/heads/main/fern/images/agent-memory-comparison/05-letta-stateful-architecture.svg" />

Letta was built by the researchers behind MemGPT, which pioneered the concept of treating LLM context management like an operating system manages memory. The platform provides sophisticated memory management that lets agents learn how to use tools, evolve personalities, and improve their capabilities over time.
...
<img src="https://raw.githubusercontent.com/letta-ai/letta/refs/heads/main/fern/images/agent-memory-comparison/agent-development-environment.png" />
...
Letta is also fully interoperable with other memory solutions. You can use Mem0, Zep, Cognee, or any other tool within Letta agents through custom Python tools or MCP (Model Context Protocol) servers. This lets you combine Letta's stateful architecture with specialized memory systems for specific use cases.

## Letta's AI Memory SDK

The [AI Memory SDK](https://www.piwheels.org/project/ai-memory-sdk/) is a separate solution from Letta (though built by the same team) that provides pluggable memory for developers who want memory capabilities without adopting a full stateful agent platform.

### What is Letta's AI Memory SDK?

The AI Memory SDK uses Letta agents (subagents) under the hood for memory management. You get the benefits of Letta's memory handling in a lightweight package that integrates with your existing LLM setup, without needing to adopt the full stateful agent infrastructure.
...
## Choosing the Right Memory Solution for Your Agent

Each of these solutions takes a different approach to agent memory. The right choice depends on your use case, existing architecture, and how much control you need over memory management.

**Choose Mem0 when:**

You want a simple drop-in solution that works with your existing agent framework. Mem0 requires minimal code changes and handles memory extraction and consolidation automatically. Good for adding memory to chatbots, customer support agents, or personal assistants where you need user/session/agent level memory organization.

Best for developers who want memory without architectural changes to their application.

**Choose Zep when:**

Facts in your domain change over time and you need to track that history. Zep's temporal knowledge graphs excel when information evolves (user preferences change, job roles update, locations shift) and you need both current truth and historical context.

Particularly valuable for enterprise applications that combine conversational data with business data, or when you need to query "What did the user prefer in Q1 vs Q3?"

Best for applications where temporal awareness and data evolution matter.

**Choose Cognee when:**

You need semantic understanding of relationships between concepts. Cognee's knowledge graph construction helps when working with domain-specific knowledge where understanding relationships is as important as retrieving information.

Good for applications processing technical documentation, research papers, or complex knowledge bases where "What are the dependencies?" is as important as "What is X?"

Best for developers who value semantic understanding.

**Choose Letta Platform when:**

You're building agents that need to truly evolve over time. Letta's stateful architecture works best for long-running agents that need to learn how to use tools better, develop consistent personalities, or improve their behavior based on experience.

The platform approach makes sense when you want infrastructure that manages agent state for you, complete with a visual development environment and sophisticated memory programming primitives.

Best for building production agent applications where agents are first-class entities, not ephemeral function calls.

**Choose AI Memory SDK when:**

You love Letta's memory management but don't need the full stateful agent platform. The AI Memory SDK gives you pluggable memory capabilities you can add to existing applications without adopting new infrastructure.

Good for developers who want more sophisticated memory handling than simple vector storage but aren't ready to commit to the stateful agent paradigm.

Best for adding memory to existing applications with minimal architectural impact.

**Interoperability Note:**

Letta is fully interoperable with these other tools. You can use Mem0, Zep, or Cognee within Letta agents through custom tools or MCP (Model Context Protocol) servers. Letta's platform can incorporate external memory systems as additional data sources while maintaining its own core memory architecture.
...
- **Mem0**: [docs.mem0.ai](https://docs.mem0.ai/) - Complete API reference and integration guides
- **Zep**: [help.getzep.com](https://help.getzep.com/) - Platform docs and Graphiti library reference
- **Cognee**: [cognee.ai](https://www.cognee.ai/) - Getting started guides and examples
