...
[Letta Identities](/guides/agents/multi-user) are how Letta manages multi-user applications. An Identity represents a user in your application and links them to their agents. Each Identity has an `id` (generated by Letta), an `identifierKey` (your application's user identifier), a `name` (human-readable label), and an `identityType` (usually "user").
