...
When building with Letta, you need to understand where it fits in your stack. Letta is an AI runtime that manages agent memory and conversations. This guide shows you how Letta integrates with your UI layer (Next.js) and database (Supabase) to build a complete application architecture.

You'll take Letta's Next.js chatbot example, which has no backend, and add Supabase to create a production-ready application with persistent storage and multi-user support. By the end, you'll understand how each piece works together.
...
- A [Letta Cloud](https://app.letta.com) account (free tier works)
- A [Supabase](https://supabase.com) account (free tier works)
- Basic familiarity with Next.js, React, and TypeScript

We'll start with [Letta's chatbot example](https://github.com/letta-ai/letta-chatbot-example) and integrate Supabase step by step.
...
Letta is the AI runtime. It manages your agents, their memory, and conversation state. When you create an agent or send a message, Letta processes it and maintains the agent's memory across conversations.
</Accordion>
...
These layers communicate in a specific order. The frontend talks to Next.js API routes. The API routes talk to both Letta (for AI processing) and Supabase (for data storage). Letta and Supabase don't talk directly to each other. Your API routes coordinate between them.
...
Update the default agent configuration to use models compatible with Letta Cloud. Open `default-agent.json` in the project root and change the model settings:
...
The `letta/letta-free` model isn't available on Letta Cloud. Letta Cloud requires the `provider/model` format (like `openai/gpt-4o-mini`).

This starter project already has a working chatbot interface built with Next.js and the Vercel AI SDK. It uses Letta to create agents and have conversations, but lacks persistent storage and multi-user support. We'll add those features by integrating Supabase.
...
<Note>
This guide uses Letta Cloud, not a local Letta server. Make sure to set `LETTA_BASE_URL` to `https://api.letta.com` (the default in the cloned project points to localhost).
</Note>
...
Click **Run** to execute the schema. You should see "Success. No rows returned" in the results panel.
...
This file reads your environment variables and creates a Supabase client. The error checks ensure you'll see a clear message if credentials are missing. Next.js automatically loads `.env` files, so we don't need additional configuration.
...
Letta Identities are how Letta manages multi-user applications. An Identity represents a user in your application and links them to their agents. Each Identity has an `id` (generated by Letta), an `identifier_key` (your application's user identifier), a `name` (human-readable label), and an `identity_type` (usually "user").
...
<Steps>
  <Step title="Add Supabase import to helpers">
  ...
  <Step title="Add getUserFromRequest function">
  ...
  <Step title="Update validateAgentOwner function">
  ...
  <Step title="Remove getUserTagId function">
...
<Steps>
  ...
  <Step title="Update getAgents function">
  ...
  <Step title="Update createAgent function">
...
<Steps>
  <Step title="Add Supabase import">
  ...
  <Step title="Update deleteAgentById function">
...
<Steps>
  ...
  <Step title="Add onFinish callback to streamText">
...
<AccordionGroup>
<Accordion title="Add archival memory for long-term storage">
Connect a vector database to store conversation history beyond what fits in the agent's context window. Letta's archival memory lets agents search through past conversations semantically:

```typescript
// Load conversation history into archival memory
await client.sources.create({
  name: `agent-${agentId}-history`,
  description: 'Past conversations with this agent'
})

// Attach to agent
await client.agents.attachSource(agentId, sourceId)
```

Your Supabase `messages` table provides the raw data, and Letta's archival memory makes it semantically searchable by the agent.
</Accordion>
...
Use Letta's shared memory blocks to give multiple agents access to common information. Store references to shared blocks in Supabase:

```sql
CREATE TABLE shared_memory_blocks (
  id UUID DEFAULT gen_random_uuid() PRIMARY KEY,
  letta_block_id TEXT NOT NULL,
...
When creating agents, attach the organization's shared blocks for consistent knowledge across all agents.
</Accordion>
...
<CardGroup cols={2}>
  <Card
    ...
    href="/guides/agents/archival_memory_overview"
    iconPosition="left"
  >
  ...
  <Card
    ...
    href="/guides/agents/custom_tools"
    iconPosition="left"
  >
  ...
  <Card
    ...
    href="/guides/agents/memory_blocks"
    iconPosition="left"
  >
