...
The `createUser` function inserts a new user row with links to both the cookie UID and the Letta Identity. The `getUserByCookieUid` function retrieves a user by their session cookie. The special error code, `PGRST116,` occurs when no rows are returned, which we handle by returning `null` instead of throwing an error.
