...
<Steps>
  ...
  <Step title="Get Your Cluster URL and API Key">
    ...
    - **Cluster URL (REST Endpoint)**: `https://xxxxx.weaviate.network`
...
Create a new file named `tools.py` with the implementation for your chosen provider:
...
1. **Register the tools** - Upload your memory functions to Letta's tool registry
2. **Define the agent's persona** - Provide instructions on when to store and retrieve memories
3. **Create the agent** - Link the tools and configure environment variables for database access
...
Now we'll interact with the agent to test memory storage and retrieval. Create a file named `chat.py`:
...
<Steps>
  ...
  <Step title="Open Your Agent">
    Click on your memory assistant agent to open it in the playground.
